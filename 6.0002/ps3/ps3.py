...
class RectangularRoom(object):
    ...
    def __str__(self):
        """
        Prints out list of tile co-ordinates and dirt associated with each tile
        """
        result = ""
        for item in self.tiles:
            result += str(item)
            result += " Dirt amount: "
            result += str(self.tiles[item])
            result += "\n"
        return result
...
class FurnishedRoom(RectangularRoom):
    ...
    def get_random_position(self):
        ...
        """
        while True:
            rand = Position(random.uniform(0, self.width), random.uniform(0, self.height))
            if not self.is_position_furnished(rand):
                return rand
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials, robot_type):
    ...
    for trial in range(num_trials):
        ...
        ticks = 0
        bots = []
        sim_room = EmptyRoom(width, height, dirt_amount)
        for bot in range(num_robots):
            bots.append(robot_type(sim_room, speed, capacity))
        while float(sim_room.get_num_cleaned_tiles()) / float(sim_room.get_num_tiles()) < min_coverage:
            for bot in bots:
                bot.update_position_and_clean()
            # anim.update(sim_room, bots)
            ticks += 1
        num_ticks.append(ticks)
        # anim.done()
    return float(sum(num_ticks)) / len(num_ticks)
