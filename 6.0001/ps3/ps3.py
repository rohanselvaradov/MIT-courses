    ...
    for char in check_word:
        if char == "*":
            for letter in VOWELS:
                temp_check_word = check_word.replace("*", letter)
                if temp_check_word in word_list:
                    return True
            return False
        if char not in check_hand:
            return False
        else:
            check_hand[char] -= 1
            if check_hand[char] == 0:
                del check_hand[char]
    if check_word not in word_list:
        return False
    return True
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    while int(hands) > 0:
        ...
        display_hand(hand)
        substitute = input("Do you want to substitute a letter? ")
        if substitute.lower() == "yes" or substitute.lower() == "y":
            letter = input("Please enter the letter you wish to substitute: ").lower()
            new_hand = substitute_hand(hand, letter)
            hand = new_hand
        temp_score = play_hand(hand, word_list)
        if not replayed:
            replay = input("Do you want to replay this round? (yes/no) ")
            if replay.lower() == "yes" or replay.lower() == "y":
                replayed = True
                temp_score_2 = play_hand(hand, word_list)
                print(temp_score, temp_score_2)
                if temp_score > temp_score_2:
                    score += temp_score
                else:
                    score += temp_score_2
            else:
        ...
        hands -= 1
    print("_ " * 20)
    print("Total score in all rounds:", score, "points")
